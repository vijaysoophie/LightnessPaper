...
Any variation resulting from the poisson nature of the photoreceptor response or due the image rendering software is negligible, and can be ignored.

(DO WE WANT TO GET AN ESTIMATE OF THE CLOUD SIZE FROM THE ALLOWED VARIATION IN THE SPECTRAL SHAPE?)
...
\subsection{Case 1a: This will be the case of background variation with fixed luminance}

\subsection{Case 2: Luminance can be estimated from contrast between target and background}
The AMA receptive fields learnt using the photoreceptor response does a very poor job in predicting the target object luminance (Fig.~\ref{fig:isomerizationFails}) for images of case 2. 
...
While, the projection along the first two receptive fields shows an overlap between the stimuli at higher luminance values (Fig.~\ref{fig:constrastWorks}), the stimuli are better separated if the projection is considered within a subspace that has more receptive fields. 
Fig.~\ref{fig:RMSEvsNFilters} shows the relative error (Methods) of estimates as a function of the of the number of receptive fields considered for luminance estimation for the cases considered in this work. 
As expected, relative error reduces with increasing number of receptive fields. 
...
We have used the first 8 receptive fields to estimate the luminance in this case (see Fig.\ref{fig:RMSEvsNFilters}). 
...
The relative error of estimation is ~17\%, which low compared to that of the naive model (~42\%) or the linear model on center pixel (~21\%).
