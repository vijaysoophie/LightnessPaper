...
% JOHANNES ARE THERE ANY OTHER PAPERS YOU WOULD LIKE TO CITE
...
To construct an ideal observer model for our task, two steps are required. First, we use AMA to learn the filters that are optimal for the task. Second, we determine how to optimally decode the task relevant variable from the filter responses to arbitrary stimuli. 

{\bf Learning optimal filters:} AMA \cite{geisler2009optimal,burge2017accuracy} is a task specific Bayesian method for dimensionality reduction. When provided with a labeled training set, a response noise model, and a cost function, AMA returns the linear receptive fields in rank order that are optimal for performing a task. The optimal filters that are returned by AMA are the filters whose noisy responses to the stimuli, as given by the specified response model, minimizes the cost function over the training set when optimally decoded. 

Let us assume that there are $N_{\nu}$ possible values of the target object luminance levels ($\{Y_k: k\in[1,N_{\nu}] \}$). We represent the $l$'th image corresponding to the $k$'th level as $s_{kl}$. Let us assume that there is a set of linear filters {\textbf{\textit f}} which when acting on $s_{kl}$ gives the response ${\textbf{\textit R}}_{kl}$. Let's say we have a decoder $g$, that uses the information of the response noise model, to return the Bayes optimal estimate of the luminance level ($\hat{Y}_k$) corresponding to the stimuli $s_{kl}$. Given this estimate, one can define a cost function $\mathcal{C}_{kl}(Y_{kl},\hat{Y}_{kl})$ corresponding to specific response ${\textbf{\textit R}}_{kl}$ and an avearge cost $\bar{\mathcal{C}}_{kl}(Y_{kl},\hat{Y}_{kl})$ corresponding to the average response $r_{kl} = \langle{\textbf{\textit R}}_{kl}\rangle$. 
...
In this work, we assume the noisy response model to be a multivariate gaussian. For a stimuli $s_{kl}$, the noisy response vector ${\textbf{\textit R}}_{kl} = \left[{\textbf{\textit R}}_{kl,1},{\textbf{\textit R}}_{kl,2},\dots,{\textbf{\textit R}}_{kl,q}\right]$ to the set of filters ${\textbf{\textit f}} = \left[{\textbf{\textit f}}_{1},{\textbf{\textit f}}_{2},\dots,{\textbf{\textit f}}_{q}\right]$ is given by $\mathcal{N}({\textbf{\textit r}}_{kl}, \Sigma)$, with mean response vector ${\textbf{\textit r}}_{kl} = \left[{\textbf{\textit r}}_{kl,1},{\textbf{\textit r}}_{kl,2},\dots,{\textbf{\textit r}}_{kl,q}\right] = \left[{\textbf{\textit f}}_{1}\cdot s_{kl},{\textbf{\textit f}}_{2}\cdot s_{kl}\dots,{\textbf{\textit f}}_{q}\cdot s_{kl}\right]$ and the covariance matrix as the diagonal matrix $\Sigma$ with variances $\left(\sigma_1^2, \sigma_2^2,\dots, \sigma_q^2 \right)$. We assume $\sigma_i^2 = \alpha |r_i| + \sigma_0^2$, $|{\textbf{\textit f}}_{kl,i}|=1$ and $s_{kl}$ to be contrast normalized, i.e., ${|s_{kl}|=1}$.
...
\subsection{Case 1: Target luminance can be estimated directly from the photoreceptor response}
We used AMA to learn a set of linear receptive fields that are optimal for estimating target object luminance using stimuli in the training set of case 1. 
Fig.~\ref{fig:case1RFResponse} shows the projection of the photoreceptor response along the first two AMA receptive fields. 
...
Fig.~\ref{fig:case9Results} shows the estimated luminance v/s the actual luminance assigned to the target object for the test images in case 1. 
...
\subsection{Case 3: Luminance can be estimated from contrast between target and background}
For images of case 2, the AMA receptive fields learnt using the photoreceptor response does a very poor job in predicting the target object luminance (Fig.~\ref{fig:isomerizationFails}). 
