...
\subsection{In the absence of illuminant and background variation, object lightness can be estimated directly from the photoreceptor response}

We used AMA to learn a set of linear receptive fields that are optimal for target object lightness estimation for images of case 1. 
Fig. \ref{fig:renderingIsomerizationTargetColor} shows the projection of the photoreceptor response along the first two receptive fields. 
Clearly, there exists a subspace along which the photoresponse response separates according to the luminance level assigned to the target object. 
The target object luminance of the images in the test set can thus be obtained by projecting the corresponding photoreceptor response along these AMA receptive fields. 
Fig.\ref{fig:case9Results} shows the estimated luminance for the test images in case 1 v/s the actual lightness assigned to the target object for AMA. 
We also show the estimates of two null models: a naive method that assigns the average luminance across the training set to all images, and a linear model based on the photoreceptors corresponding to the center pixel on the target object. 
...
Each cloud in Fig.\ref{fig:renderingIsomerizationTargetColor} corresponds to the photoreceptor response to the images with target luminance level at a fixed value. 
The variation within the cloud results due to the difference in shape of the target object reflectance spectra at the same luminance. 
This variation dominates any variation coming from the noise in photoreceptor response or the noisy rendering of the images by the software. 
Fig.\ref{fig:renderingAndIsomerization} shows the projection of photoreceptor response corresponding to 1000 images where, in addition to the illumination and background spectra, we also fixed the relative shape of the target object surface reflectance. 
Thus, for these images the target object had the same hue and chroma at all luminance levels.
...
\centering
    ...
    ~
        \begin{subfigure}[b]{0.3 \textwidth}
        \includegraphics[width=\textwidth]{../Figures/resultsFigure/contrastWorks.pdf}
