...
Within each row, the object luminance of the spheres is constant; we define object luminance as the luminance of the light that would be reflected by the object's surface under standard illuminant CIE D65.  The values on the left of each row provide the corresponding object luminance. We cast the problem of computational luminance constancy to be that of estimating object luminance from the image, across variation in other scene factors: variation in relative surface reflectance, variation in illumination, and variation in the reflectance of the other objects in the scene.}
 \end{figure}
...
% Autonmous vehicle work - ask BAW
 ...
 {\em virtual natural scenes}. 

 we also model the initial steps of the human visual system, so that our supervised learning methods are applied not to image pixels but rather to modeled responses of the foveal cone photoreceptor mosaic. 
...
The light that reflects from objects to the eyes depends on many factors. These include the surface reflectance, texture, material and geometry of the objects, the spectra of the illuminants, and the position of the observer.
We developed a rendering pipeline that allows us to specify models of naturalistic scenes. The pipeline harnesses computer graphics to render, for each scene, a corresponding image.  The renderer traces the flow of light through the scene as it interacts with object surfaces, so that the images are physically accurate.
Because each image is rendered from a known scene description, each image pixel can be labeled with the surface reflectance of the corresponding scene object.
...
The purpose of VWCC is to allow us to specify and render a large set of diverse scenes.

VWCC includes a collection of base scenes (Fig.~\ref{fig:baseScenes}).
Base scenes specify an arrangment of objects and light sources, and provide a starting point for constructing more complex scenes via the placement of additional objects and light sources. To guide where to put such additional objects and light sources, base scenes are annotated with meta data. To render an image of a scene, it is necessary to specify a viewpoint. The meta data also includes information to guide selection of the viewpoint.

VWCC contains a collection of objects that may be inserted into base scenes (Fig.~\ref{fig:libraryWithTarget}).
It also allows positioning of additional point and area lights.
In addition, VWCC provides the user with control over key scene properties. These include the position, size and pose of inserted objects, the reflectance of surfaces, and the spectra of illuminants (Fig.~\ref{fig:targetPositionVariation}).
% REORDER FIGURE 5 COLUMNS d -> e -> a -> b -> c.

Thus, along with distributions that characterize parameters such as the number, size, shape and orientation of the inserted objects and light sources, we have a simple statistical model of the object and light source geometry of natural scenes. 
...
We used AMA to learn a set of linear receptive fields that are optimal for estimating target object luminance from the model cone photoreceptor response to the images from this case.
...
Thus, the target object luminance can be obtained by projecting the photoreceptor response along these AMA receptive fields. 
...
This is because the isomerization noise is directly proportional to the light reflected from the target object and is higher at higher values of the target object luminance.
...
\centering
    ...
    \end{subfigure}
    \caption{{\bf Case 2:} (a) Photoreceptor response projected along the first two AMA receptive fields. (b) Estimated v/s assigned target object luminance for the image in case 2. Solid lines show the mean estimate, the filled region in light color  shows one standard deviation.}
...
For the spectral variations of case 3, the AMA receptive fields learnt on the photoreceptor response to the images does a very poor job in predicting the target object luminance (Fig.~\ref{fig:isomerizationFails}). 
...
Thus, the AMA receptive fields learnt on the contrast normalized photoreceptor response (see Methods) can be used to predict the target object luminance (Fig.~\ref{fig:constrastWorks}). 
Fig.~\ref{fig:case10Results} shows the estimated target object luminance v/s the assigned luminance for the images in case 3 obtained using the contrast normalized representation. 
...
%% Figure Case 3: Isomerization Fails
\begin{figure}
\centering
    \begin{subfigure}[b]{0.3	 \textwidth}   
