...
\abstract{Human visual system has the ability to stably perceive the color of an object under variations in the reflected light due to surround factors. This is a behaviorally significant invariance detection ability whose biological mechanism remains unknown. To understand the computational principles that can lead to such invariant perception, here we study the perception of object lightness under spectral variations. Lightness is the attribute of the 3D color space that determines the brightness of a color. Specifically, we study the effects of variations in reflectance and illumination spectra of the objects and lights sources in a scene on the perceived lightness of an object. We have developed a software to generate naturalistic hyper-spectral images of 3D scenes with precise control over the geometrical and spectral properties of the objects and lights in the scene. The software also produces the response of the early visual system to these hyper-spectral images. We label the images with the lightness of a specific object in the scene and use supervised learning methods to identify the computations that lead to stable lightness perception. Our analysis shows that for simple spectral variations, like the ones that allow variations only in the spectrum of the target object, or of the background object, or of the light source, the standard lightness can be recovered by through transformations of the reflected light. When all spectra are allowed to vary simultaneously, while it is not easy to recover the lightness through simple transformations, a decoding sheme that compares the light from the target and the surround, and properly adds the response of the L,M and S cones can recover the lightness within about 15\% RMSE.}
...
The variations in the reflected light can result from either geometrical factors (like shape, size, relative position of objects, etc.) or spectral factors (such as the illumination spectra, surface reflectance, diffusivity, etc.). Here, we focus on the spectral factors. Specifically, we study the effects of variations in the surface reflectance spectra and the illumination spectra on lightness perception  (Fig.\ref{fig:introExampleFigure}). We are interested in identifying the computations that can lead to stable lightness perception.

\begin{comment}
\begin{figure}
\centering
\includegraphics[width=\textwidth]{approach.png}
\caption{{\bf Approach:} We have employed a supervised learning approach to study color constancy. We first generate a large dataset of hyperspectral images labeled with the lightness level of the target object. We generate images at multiple lightness levels, with images at the same lightness level having variable object reflectance spectra and illumination spectra. These hyperspectral images are used to simulate the response of the visual system. We use isetbio toolbox to model the optics and physiology of early vision and produce the response of the cones in the retina. We use supervised computational learning methods on this labeled dataset of cone-responses to estimate the lightness of the target object. The performance of the supervised method is calculated using a separate test dataset.} 
\label{fig:approach}
\end{figure}
\end{comment}
...
\end{align}
and the total cost associated with the training set is $\bar{\mathcal{C}} = \frac{1}{N} \sum\limits_{k,l}\bar{\mathcal{C}}_{kl} = \frac{1}{N} \sum\limits_{k,l}\langle \mathcal{C}_{kl}\rangle_{{\textbf{\textit R}}_{kl}}$. The optimal AMA filters are obtained by minimizing $\bar{\mathcal{C}}$ over the filter set ${\textbf {\textit f} }$ (Eq.\ref{eq:fopt}). The filters are obtained using gradient descent.
...
We have studied the effects of spectral variations on perceived lightness. These can be categorized into three types: variations in the shape of target object reflectance spectra, variations in the shape of background object reflectance spectra, and the variations in the shape of the spectral power distribution of light sources. Here, we present the results for three cases (Fig.\ref{fig:studiedCases}): 1. effects of variations in target object spectra (Fig.\ref{fig:targetVarying}), 2. effects of variations in target object spectra and the light source spectra (Fig.\ref{fig:targetIlluminantVarying}), and 3. effects of variations in all three spectra (Fig.\ref{fig:allSpectraVarying}). The cases with other types of variations are qualitatively similar to one of the three cases discussed here (see Discussion).


\section{Results}
\subsection{Case 1}
Fig.~\ref{fig:case9AllResults} shows the receptive fields and the the AMA lightness estimates for the three cases discussed above. We show the target object lightness estimates for there different methods: linear regression on center pixel, SVD regression and AMA. As is expected, when only the target object reflectance varies, it is easy to estimate the target object lightness. For case 2, where the light source spectra are allowed to vary, although the light captured by the cones vary from image to image, the contrast between the target and the background remains fixed and can be used to estimate the target object lightness. Thus, the SVD-LDA and AMA still provide good estimates of the target object lightness. In case 3, the most challenging case where all the spectra are allowed to change, AMA still does a reasonable job of estimating the target object lightness. AMA mean estimates lie close to the diagonal x=y line.
