...
Fig.~\ref{fig:case1RFResponse} shows the responses of the first two receptive fields to all the image patches in the training set.
...
Decoding performance for the test set is shown in Figure (WHATSI).
As expected, the LRV estimates obtained by decoding the AMA receptive field responses are veridical.
Figure (WHATSI) also shows the performance of the baseline linear regression method, again trained
...
%ont along the first two optimal AMA receptive fields. 
...
% shows the estimated luminance v/s the actual luminance assigned to the target object for the test stimuli in condition 1. 
