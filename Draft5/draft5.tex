...
\caption{{\bf Change of relative RMSE with number of receptive fields (RFs) and size of training set:} (a) Change of relative RMSE for Condition 3 with the size of training set. The relative RMSE converges around 2000 images. To make this plot, the 300 image test set for Condition 3 was used in each case, while the training set size varied between 300 to 2700 images. The number of receptive fields used was varied between 1 and 6. (b) Same data as in panel a, but plotted as a function of number of receptive fields. Performance converges around 3 RFs. The error bars in each panel show +/- 1 SEM, with the variation taken across multiple runs with different sets if starting points for the numerical search used in our implementation of AMA. }
 \label{fig:RMSEvsRF}
\end{figure}
...
In contrast to the Condition 1 RFs, there is also systematic contribution of cone excitations from background object locations, with both positive and negative
contributions from background objects.
...
The third receptive field for Condition 3 places weight on the S cone excitations. 
...
Finally, we note that there is some run-to-run variation in the structure of the optimal receptive fields that depends on the set of randomly-chosen starting
points used for the numerical search used in our implementation of AMA.
This variation also suggests that one should not over interpret details of the structure of the receptive fields.
Such run-to-run RF variation leads only to very small changes in estimation error (see error bars in Figure~\ref{fig:RMSEvsRF}).
...
Supported by RO1 EY100106 (DHB), Simons Foundation Collaboration on the Global Brain Grant 324759 (DHB) and the University of Pennsylvania Computational Neuroscience Initiative (VS).  Prelminary versions of this work were presented at the Computational Cognitive Neuroscience Conference (2017) and the Annual Meeting of the Society for Neuroscience (2017).
