...
\title{Luminance Constancy Learned from Labeled Naturalistic Images}
%% Luminance constancy in naturalistic scenes
%% Learned luminance constancy: something
%% 

\abstract{
The human visual system supports stable percepts of object color even though the light that reflects from object surfaces varies significantly with the scene illumination.
...
We show that a decoding scheme used learned linear filters to process a contrast representation of the cone excitations can recover target object LRV to within about 13\% RMSE.
...
It is possible, however, to obtain such descriptors by jointly analyzing the light reflected from multiple objects in the scene.
As a consequence, the performance of color constancy algorithms will be affected both by variation in the illumination and in the surface reflectance of other objects in the scene,
and thus algorithms need to be evaluated with respect to variation in both of these factors \cite{BrainardWandellRetinex,BrainardFreeman}. 
There has also been computational work that generalizes to more complex viewing geometries \cite{funt1988color, barron2012color}.
...
The package builds on our RenderToolbox4 package (\href{http://rendertoolbox.org}{http://rendertoolbox.org}) \cite{heasly2014rendertoolbox3},
...
By incorporating statistical models of object surface reflectance and daylight illumination, 
...
The datasets for Conditions 1 and 2 consiseted of 1000 scenes and images, 100 for each of 10 target object LRV values. 
...
For each LRV value, we generated a different relative target surface reflectance for each scene. 
% Figure 4
\begin{figure}
...
scenes in the datasets. The geometry is shown in Figure~\ref{fig:3DScene} below.
...
Our statistical model approximates normalized spectra from the Granda dataset using the first 6 principle components of the normalized measurements from that dataset.
...
This range corresponds approximately to that of natural daylight viewing.
...
To generate the target object reflectance at a particular LRV, the generated reflectance spectrum was 
...
In primary visual cortex, foveal receptive fields have a spatial extent of approximately 1 degree of visual angle \cite{gattass1981visual, gattass1988visuotopic}. 

We modeled a visual system by typical optical blurring (including axial chromatic aberration) in the formation of the retinal image \cite{marimont1994matching}, and spatial sampling by an interleaved rectangular mosaic of long (L), middle (M)  and short (S) wavelength-sensitive cones (\citeNP{brainard2015color}; Fig.~\ref{fig:croppedImageWithMosaic}). 
...
Once the AMA receptive fields have been learned, we need to develop a general decoder that can be applied to arbitrary test images.
A separate decoder is necessary because the decoder used to learn the receptive fields depends on the response mean and response variance of the filters to every labelled stimulus in the training set \cite{geisler2009optimal,burge2017accuracy}.
To proceed, first we use the AMA receptive fields and the training dataset to find the conditional distributions of the receptive field responses, where the conditioning is on the labelled target object LRVs.
...
contribution.
%% DHB: We need to come back to this paragraph after we finalize the RFs to be shown in Figure 10c and 12c,
% because the third RF does have S cone contribution.
...
The specific geometry of the receptive fields should not be taken to be a general feature of the human visual system, as the RF geometry arises as a consequence
...
%% DHB: There are all sorts of problems with the citations.
% a) Journal and book case should be (e.g.) "Journal of Vision" not (e.g.) "Journal of vision".  This is botched in many places.
% b) In some cases we appear to be citing things that are one page long.  I think many of these are papers that have more than
% one page, and those instances should be fixed.  In other cases, we may be citing abstracts and I would prefer not to cite those
% unless there is a real need to.  
% c) Sometimes page numbers are given by (e.g.) "112-173", other by (e.g.) ", yet others by (e.g.) "(pp. 112-173)".  Pick one style
% and stick with it.
% d) There are other glitches.  For example, the title of Goncalves and Welchman starts "?what not?".  Proofread carefully and fix up.
