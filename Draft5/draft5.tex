...
%% DHB, VS: Maybe try to add a sentence saying why it is important.
}
...
Specifically, we study variation in target object relative surface reflectance, variation in illumination spectrum, and variation in the surface reflectance of the other objects in the scene.}
 \end{figure}
%% DHB: Need to decide and then change with search through document: "relative reflectance spectrum" or "shape of reflectance spectrum".
% I prefer relative, because "shape" in a vision paper generally refers to object shape.
% And we found at least one instance where trying to use "shape" led to an awkward construction.
...
%% DHB: Let's get the scaling and tone mapping used for these under control, so we know what it is.
%% DHB: Maybe also say that the illumination scaling is much smaller than the range used in the study, so that it is possible to see it with common scaling.
...
\end{figure}
%% DHB: Need to generate these so that we display them all with a common scale factor and tone mapping. Then say so in the caption once it is done.
...
The LRV values were equally spaced between 0.2 and 0.6. For each LRV value, a different target surface reflectance spectra for each of the 100 scenes. Thus, for each target object LRV value, the shape of the target object surface reflectance spectra could differ.
...
%% VS: Needed a reference from Johannes.
...
% JDB: paragraph seems disjoint. Talk to David and Vijay.
% DHB and VS: We think it is fine but are leaving the comment so we can call remember to discuss.
...
%% DHB: Wouldn't it make more sense to use MSE and posterior mean for both learning and decoding, since we are reporting relative RMSE as our performance measure?
...
% from the distribution, though.  Clarify?  Or actually do that?
%
...
contrast-based representations are effective for supporting color constancy when only illumination spectra vary (e.g. REFS 1).
However, contrast-based representations are less effective at supporting constancy when the spectra of background objects in the scene vary (e.g. REFS 2).
%% REFS 1:
% [1] Land, E.H. 1986 An alternative technique for the computation of the designator in the retinex theory of color vision. Proc. Natl. Acad. Sci. 83, 3078-3080.
%
% REFS 2:
% [1] Brainard, D.H. & Wandell, B.A. 1986 Analysis of the retinex theory of color vision. Journal of the Optical Society of America A 3, 1651-1661.
...
This effect is shown the right panel of Figure REF, which also illustrates the effect of training set size on estimation performance. 
%% DHB: Check rewrite of last two sentences: OK?
%% VS to add REF to last sentence above.
...
%% JDB: ADD REFERENCE TO Burge, Fowlkes, Banks (2010). ?Natural scene statistics predict how the figure-ground cue of convexity affects human depth perception?. Journal of Neuroscience, 30(21): 7269-7280.
% JDB: ADD REFERENCE TO Girshick, Landy, Simoncelli (2011). Cardinal rules: visual orientation perception reflects knowledge of environmental statistics. Nature Neuroscience, 14(7), 926-932. 
%% JDB: ADD REFERENCE TO Burge, McCann, Geisler (2016). Estimating 3D tilt from local image cues in natural scenes. Journal of Vision, 16(13):2, 1?25.
%% JDB: ADD REFERNECE TO Goncalves & Welchman (2017).  "What Not?? Detectors Help the Brain See in Depth. Current Biology, 27, 1-10
