...
Down each column, the sphere's luminance reflectance value (LRV) varies, but the shape of the reflectance spectrum is held fixed.
...
%		\includegraphics[width=\textwidth]{../FiguresDraft5/Figure2/Figure2_a.png}
    ...
    \end{subfigure}    
    \caption{{\bf Dataset Conditions 1-3:} sRGB renderings of illustrative images from the datasets for Conditions 1, 2, and 3. Five images are shown for each luminance reflectance value (LRV). Each dataset had at least 100 images for each LRV. We studied three types of spectral variations. (a) Condition 1: variable target object relative surface reflectance spectrum, fixed light source spectra, fixed background object spectra (same as Fig.~\ref{fig:introExampleFigure}).
(b) Condition 2: variable target object relative surface reflectance spectrum, variable light source spectra, fixed background object spectra, (c) Condition 3: variable target object relative surface reflectance spectrum, variable light source spectra, variable background object spectra. The spheres in each row of subplots have the same LRV, while the spheres in each column of each subplot have the same relative surface reflectance.  Across the three sets of subplots, spheres in corresponding locations have the same surface reflectance. In all three subplots, the overall lights source spectra scale factors (see \nameref{Methods}) were drawn from a uniform distribution on the range [0.5, 1.5]. This is smaller than the variation we studied, but allows us to show the variation within the dynamic range available for computer monitors and printed figures. The sRGB values for all three subplots were normalized using a common scale factor prior to gamma correction.} 
\label{fig:studiedCases}
\end{figure}
%%JDB: ADD "Luminance Reflectance Value (LRV) label to the left-most y-axis.
