...
Optimally decoding target object LRV from the responses of a small learned set of task-specific linear filters that operate on a contrast representation of the cone excitations yields estimates that are within 13\% of the correct LRV.
...
% JDB: WE DIDN'T DEFINE RMSE SO IT MIGHT BE BEST TO AVOID USING THE ACRONYM HERE.
...
% JDB: LAST SENTENCE DANGLES. HOW BOUT WE PUT IT LOWER DOWN IN THE MANUSCRIPT?
...
% JDB: THE ABOVE SENTENCE IS CORRECT?
...
% JDB: THE NUMBERS [0.15, 150] ARE ARBITRARY. THE RELEVANT INFO IS THE NEXT SENTENCE.
% To restore the overall intensity variation in the Granada dataset, we scaled each randomly generated spectrum with a random number sampled from a log-uniform distribution in the range [0.15, 150].
...
We modeled a visual system having typical optical blurring (including axial chromatic aberration)\cite{marimont1994matching}, and typical spatial sampling with an interleaved rectangular mosaic of long (L), middle (M)  and short (S) wavelength-sensitive cones (\citeNP{brainard2015color}; Fig.~\ref{fig:croppedImageWithMosaic}). 
...
% JDB: SHOULD WE SWAP 'SIGNAL TO NOISE' FOR 'AVAILABLE INFORMATION'.
...
% , on the assumption that all images in the training set are equally likely.
% JDB: CLAUSE SEEMS UNNECESSARY
...
% JDB: DOUBLE CHECK THAT THE ADDITIONIAL CLAUSE AT END OF LAST SENTENCE IS CORRECT.
...
%% JDB: VIJAY, If you recompute the reduction in AMA cost for RFs 1, 2, and 3 in order and then compute the reduction in AMA cost for RFs 1, 3, and 2 in order, which of the two orders gives lower cost with two filters? 
...
% % JDB: WE ALSO NEED TO WRITE SOMETHING SOMEWHERE ABOUT WHY THE RFs HAVE THE ODD LOOKING SHAPES THAT THEY DO.
...
The specific geometry of the receptive fields should not be taken to be a general feature of the human visual system, as the RF geometry arises as a consequence of the fixed scene geometry used in the training set.
