...
%DIF ADD Revision1.tex   Thu Sep  6 16:53:11 2018
\usepackage{graphicx}
...
we study how estimating a target object's light reflectance value (LRV; a measure of the light reflected from the object under a standard illuminant) depends on variation in key properties of naturalistic scenes. 
Specifically, we study how variation in target object reflectance, illumination spectra, and the reflectance of background objects in a scene impact estimation of a target object's LRV. 
...
Optimally decoding target object LRV from the responses of a small learned set of task-specific linear receptive fields that operate on a contrast representation of the cone excitations yields estimates that are within 13\% of the correct LRV.
...
This special case is called lightness constancy \mbox{%DIFAUXCMD
\cite{gilchrist2006seeing}}%DIFAUXCMD
. 
Although human lightness and color constancy are }\DIFaddend not perfect, \DIFdelbegin \DIFdel{it is }\DIFdelend \DIFaddbegin \DIFadd{they are }\DIFaddend often very good \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{FosterColorConstancy, BrainardColorConstancy}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{FosterColorConstancy, BrainardColorConstancy,adelson2000,kingdom2011lightness}}%DIFAUXCMD
}\DIFaddend . 
...
We define the computational problem of luminance constancy as that of estimating the light reflectance value (LRV) of a target object's surface reflectance function\DIFaddbegin \footnote{\DIFadd{In the literature, the term lightness constancy is generally used to denote color constancy for the special case when stimuli are restricted to be achromatic. This condition does not apply to our work - we consider full spectral variation in the stimuli. We chose the term luminance constancy to denote the generalization from achromatic stimuli, while at the same time acknowledging that we are not studying the full problem of color constancy but rather the estimation of a luminance-based summary of surface spectral reflectance.}}\DIFaddend .
The LRV is a measure of the overall amount of light reflected by a surface \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{astm1121477}}%DIFAUXCMD
.
Estimating }\DIFdelend \DIFaddbegin \DIFadd{reflectance of the reference illuminant itself}\footnote{\DIFadd{LRV is related to albedo, but the concept of albedo does not incorporate the human luminosity function. It is also related to Munsell Value, but Munsell Value is defined with respect to a somewhat obsolete standard illuminant (CIE Illuminant C).}} \DIFadd{\mbox{%DIFAUXCMD
\cite{astm1121477}}%DIFAUXCMD
.
Obtaining }\DIFaddend the LRV from a surface reflectance function \DIFdelbegin \DIFdel{proceeds in }\DIFdelend \DIFaddbegin \DIFadd{requires }\DIFaddend two steps.
...
We modeled a visual system having typical optical blurring (including axial chromatic aberration\DIFdelbegin \DIFdel{)\mbox{%DIFAUXCMD
\cite{marimont1994matching}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{, \mbox{%DIFAUXCMD
\citeNP{marimont1994matching}}%DIFAUXCMD
)}\DIFaddend , and typical spatial sampling with an interleaved rectangular mosaic of long (L), middle (M)  and short (S) wavelength-sensitive cones (\citeNP{brainard2015color}; Fig.~\ref{fig:croppedImageWithMosaic}). 
The cone mosaic contained L:M:S cones in the ratio 0.6:0.3:0.1 \DIFaddbegin \DIFadd{(1523 L cones, 801 M cones, 277 S cones) }\DIFaddend and with spectral sensitivities derived from the CIE physiological standard \cite{CIE86}.
...
interpolating to locations between such standards requires strong assumptions that are often unjustified in real scenes. }\DIFdelend \DIFaddbegin \DIFadd{illuminant and surface properties at each image location is painstaking. That noted, there are useful databases for evaluation of color constancy algorithms that provide information about illumination and/or surface reflectance (e.g., \mbox{%DIFAUXCMD
\citeNP{barnard2002data,ciurea2003large, cheng2014illuminant,NascimentoFoster2016}}%DIFAUXCMD
...
\DIFaddbegin \DIFadd{A similar approach has been used previously to study the perception of lightness and specularity \mbox{%DIFAUXCMD
\cite{toscani2013optimal,wiebel2015statistical,toscani2017lightness}}%DIFAUXCMD
...
One advantage is that they allow us to work with large number of labeled images where object reflectance is precisely known at each pixel. A second advantage is that we can control the variation in distinct scene factors that might affect the difficulty of the estimation problem. This flexibility allows the study of individual scene factors as well as their combined effects. Here, we exploited this flexibility to quantify how variation in the relative reflectance spectrum of the target object, the spectrum of the illumination, and the reflectance spectrum of the background objects limit LRV estimation. We also exploited our use of rendered images to explore how the presence or absence of secondary reflections from background objects affected estimation of target object LRV. This type of question cannot be addressed using real images. The basic approach we use here can be extended to include parametric control over the amount of variation of different factors. For example, we could systematically vary the variances of the distribution over the weights that control the relative spectrum of the illumination.
...
To increase the representativeness of our rendered images, we used datasets of natural surface reflectance spectra and natural daylight illumination spectra. Although we believe these datasets provide reasonable approximations of the statistical variation in reflectance and illumination spectra, they can be extended and improved. For example, \DIFdelbegin \DIFdel{the surface reflectance datasets capture regularities in naturally occurring reflectance functions, but the surfaces in the datasets were not sampled to reflect }\DIFdelend \DIFaddbegin \DIFadd{there are additional datasets of measured surface reflectances that could be incorporated into future analyses, with some of these focused on the reflectance of objects (e.g. fruit) that are thought to be important for the evolution of primate color vision (e.g., \mbox{%DIFAUXCMD
\citeNP{sumner2000catarrhine, regan2001fruits, barnard2002data, ennis2018hyperspectral}}%DIFAUXCMD
). Another issue, not addressed by these datasets, is }\DIFaddend the relative frequency of \DIFaddbegin \DIFadd{occcurance of }\DIFaddend different reflectance spectra in natural viewing. \DIFdelbegin \DIFdel{In addition, the dataset of illuminations does not include the artificial illuminations that we now live much of our lives under. Future work with virtual image datasets will benefit from targeted efforts to characterize additional features of the natural statistics of the scene components that are varied}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeNP{attewell2007distribution} }%DIFAUXCMD
made a systematic survey, and found that a beta distribution provided a better description of the distribution of an LRV-like quantity than did a truncated Gaussian. Generalizing these measurements to better characterize the distribution of full reflectance functions remains an interesting goal}\DIFaddend . 

\subsection{Future Directions}
\DIFaddbegin 

\DIFaddend In the work presented here, we studied computational luminance constancy in virtual scenes with naturalistic spectral variation in light sources and in surface reflectance functions\DIFaddbegin \DIFadd{, with only matte surfaces in the scenes}\DIFaddend . It is natural to \DIFdelbegin \DIFdel{begin }\DIFdelend \DIFaddbegin \DIFadd{by studying }\DIFaddend with spectral variation, because this variation is at the heart of what makes luminance constancy a rich computational problem. In natural scenes, however, there are other sources of variation that add additional richness\DIFdelbegin \DIFdel{and make the problem more difficult to solve}\DIFdelend . These include variation in non-spectral properties of objects and lighting in the scene, including object texture, material \DIFaddbegin \DIFadd{(e.g. specularity)}\DIFaddend , and shape as well as lighting geometry. The methods we developed here may be generalized to study the effects of variation in these factors. \DIFdelbegin \DIFdel{One }\DIFdelend \DIFaddbegin \DIFadd{That is, one }\DIFaddend could incorporate these other sources of variation into the generation of the scenes and again learn estimators from the corresponding labeled images. A challenge for this approach will be to thoughtfully control the increase in problem complexity, both to keep compute time feasible and to ensure that it is possible to extract meaningful insight from the results\DIFaddbegin \DIFadd{. Extending the work to include variation of material may provide insights not only about luminance constancy but also for computations that relate to material perception (see \mbox{%DIFAUXCMD
\citeNP{fleming2017material}}%DIFAUXCMD
); extending the work to include variation in object shape and lighting geometry may clarify the role of object boundaries versus object interiors for providing information that supports perception of object color and lightness (see \mbox{%DIFAUXCMD
\citeNP{land1971lightness, rudd2016retinex}}%DIFAUXCMD
). We also note that there is a literature on how increasing stimulus complexity along the various lines listed above affects human color and lightness perception (e.g. \mbox{%DIFAUXCMD
\citeNP{beck1964effect,yang2001illuminant, yang2002stereo,todd2004lightness, snyder2005illumination, boyaci2003effect, xiao2008surface, kingdom2011lightness, xiao2012color, anderson2015perceptual, toscani2017lightness}}%DIFAUXCMD
), as well as the computational problem of color and lightness constancy (e.g. \mbox{%DIFAUXCMD
\citeNP{lee1986method, d1986mechanisms, funt1988color, tominaga1989standard, barron2012color, barron2015convolutional, finlayson2018colour}}%DIFAUXCMD
).
}


\DIFadd{We studied the information available for LRV estimation using a 1°x1° image patch. As noted above this choice of size was motivated in part to use a spatial scale roughly commensurate with the scale of information integration in early visual cortex. Our general methods could be extended to study larger regions, and doing so would quantify the value of spatially remote information for luminance constancy}\DIFaddend . 
