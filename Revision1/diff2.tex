...
%DIF ADD Revision1.tex   Tue Sep 18 14:21:14 2018
\usepackage{graphicx}
...
The \DIFdelbegin \DIFdel{LRV }\DIFdelend \DIFaddbegin \DIFadd{LRF }\DIFaddend is a measure of the overall amount of light reflected by a surface \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{astm1121477}}%DIFAUXCMD
.
Estimating the LRV from a }\DIFdelend \DIFaddbegin \DIFadd{relative to the reference illuminant itself}\footnote{\DIFadd{LRF is related to albedo, but the concept of albedo does not incorporate the human luminosity function.}} \DIFadd{\mbox{%DIFAUXCMD
\cite{astm1121477}}%DIFAUXCMD
.
...
%DIF <  JDB: DO WE HAVE REFERENCE INDICATING THAT 100ms IS A REASONABLE INTEGRATION TIME TO ASSUME?
%DIF <  LEAVING HANGING FOR NOW.  MAYBE LOOK AT COHN TWO-FLASH DETECTION EXPERIMENTS OR TEMPORAL
%DIF <  SUMMATION.
...
A similar approach has been used previously to study the perception of lightness and specularity \mbox{%DIFAUXCMD
...
To increase the representativeness of our rendered images, we used datasets of natural surface reflectance spectra and natural daylight illumination spectra. Although we believe these datasets provide reasonable approximations of the statistical variation in reflectance and illumination spectra, they can be extended and improved. For example, \DIFdelbegin \DIFdel{the surface reflectance datasets capture regularities in naturally occurring reflectance functions, but the surfaces in the datasets were not sampled to reflect the }\DIFdelend \DIFaddbegin \DIFadd{there are additional datasets of measured surface reflectances that could be incorporated into future analyses. Some of these datasets focused on the reflectance of objects (e.g. fruit) that are thought to be important for the evolution of primate color vision (e.g., \mbox{%DIFAUXCMD
\citeNP{sumner2000catarrhine, regan2001fruits, barnard2002data, ennis2018hyperspectral}}%DIFAUXCMD
). Another issue, not addressed by these datasets, is }\DIFaddend relative frequency of different \DIFdelbegin \DIFdel{reflectance spectra }\DIFdelend \DIFaddbegin \DIFadd{surface reflectances }\DIFaddend in natural viewing. \DIFdelbegin \DIFdel{In addition, }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeNP{attewell2007distribution} }%DIFAUXCMD
performed a systematic survey, and reported the distribution of an LRF-like quantity in natural scenes. Generalizing these measurements to better characterize }\DIFaddend the \DIFdelbegin \DIFdel{dataset of illuminations does not include the artificial illuminations that we now live much of our lives under. Future work with virtual image datasets will benefit from targeted efforts to characterize additional features of the natural statistics of the scene components that are varied}\DIFdelend \DIFaddbegin \DIFadd{distribution of full reflectance functions remains an interesting goal}\DIFaddend . 

\subsection{Future Directions}
\DIFaddbegin 

\DIFaddend In the work presented here, we studied computational luminance constancy in virtual scenes with naturalistic spectral variation in light sources and in surface reflectance functions\DIFaddbegin \DIFadd{, with only matte surfaces in the scenes}\DIFaddend . It is natural to \DIFdelbegin \DIFdel{begin }\DIFdelend \DIFaddbegin \DIFadd{start }\DIFaddend with spectral variation, because this variation is at the heart of what makes luminance constancy a rich computational problem. In natural scenes, however, there are other sources of variation that add additional richness\DIFdelbegin \DIFdel{and make the problem more difficult to solve}\DIFdelend . These include variation in non-spectral properties of \DIFdelbegin \DIFdel{objects and lighting }\DIFdelend \DIFaddbegin \DIFadd{lighting and objects }\DIFaddend in the scene\DIFdelbegin \DIFdel{, including }\DIFdelend \DIFaddbegin \DIFadd{. Examples include lighting geometry, }\DIFaddend object texture, \DIFdelbegin \DIFdel{material, and shape as well as lighting geometry}\DIFdelend \DIFaddbegin \DIFadd{object material (e.g. specularity), and object shape}\DIFaddend . The methods we developed here may be generalized to study the effects of variation in these factors. \DIFdelbegin \DIFdel{One }\DIFdelend \DIFaddbegin \DIFadd{That is, one }\DIFaddend could incorporate these \DIFdelbegin \DIFdel{other sources of variation }\DIFdelend \DIFaddbegin \DIFadd{factors }\DIFaddend into the generation of the scenes and again learn estimators from the corresponding labeled images. A challenge for this approach will be to thoughtfully control the increase in problem complexity, both to keep compute time feasible and to ensure that it is possible to extract meaningful insight from the results. \DIFaddbegin \DIFadd{Extending the work to include variation of material may provide insights not only about luminance constancy but also for computations that relate to material perception (see \mbox{%DIFAUXCMD
...
), and the computational problem of color and lightness constancy (e.g. \mbox{%DIFAUXCMD
