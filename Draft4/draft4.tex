...
If, in addition, the background object spectra also vary, the cone contrast no longer 
proportional to the target object LRV.
In this case, the estimators can predict the LRV with relative RMSE ~13\%.

The variability in the final condition results from changes in background object spectra.
Change in background object spectra can affect the estimation in two ways.
First, it will effect the estimation of average background isomerization in the contrast estimation.
Second, it can also effect the total isomerization in cones corresponding to the target object through 
secondary reflections from the background objects.
While the first effect can be minimized by a better sampling of the background, 
...
A major reason that such effects have been ignored in the study of color constancy is that we have 
lacked tools through which their effects could be quantified.
In this regard, a major contribution of this work has been to develop a computational package that
produces naturalistic images with precise control over all factors that define the scene in the image.
