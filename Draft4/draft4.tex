...
%In this paper, we consider the computational problem of color constancy. Specifically, we ask how a visual system should process the light reflected to the eye to produce percepts well-correlated with object surface reflectance.

%JDB: talk w vijay and david
...
%JDB: talk to vijay and david. condense last sentences?
%Evaluation of constancy algorithms should therefore include assessment of their performance with respect to both variation in the illumination and the surface reflectance of other objects in the scene  \cite{BrainardWandellRetinex,BrainardFreeman}. 
>>>>>>> Stashed changes
...
=======
%JDB: talk to vijay and david. odd transition. Add 2012 reference.
An alternative approach to computational color constancy, which has been less-well explored, is to use supervised statistical learning methods to develop mappings between input images and stable color descriptors \cite{barron2015convolutional}. % APPROPRIATE  CITATION?
...
%The use of supervised learning to understand perceptual performance with natural stimuli has enjoyed success in domains outside of color vision. 
...
The LRV is a measure of the overall amount of light reflected by a surface \cite{astm1121477}. % JDB: Confusing next sentence. Can we simplify?
...
LRVs range from 0 to 1. % JDB: DO LRVs REALLY RANGE FROM  0 TO 1?
...
%Each dataset consisted of 1000 scenes and corresponding images, 100 for each of 10 LRV values.
...
%Each condition had a corresponding dataset of 1000 scenes and corresponding images, 100 for each of 10 LRV values. The LRV values were equally spaced between 0.2 and 0.6 and generated 100 target surface reflectance spectra at each of these values. For each target object LRV value, the shape of the target object surface reflectance spectra could differ.
...
% JDB: Move this last sentence up?
...
% on these components obtained from the normalized Granada spectra 
...
% JDB: We haven't yet mentioned that we normalized the spectra (or how) before we do PCA. 
...
%% JDB: double check that changes are kosher.
...
% JDB: mismatch in the detail provided about the two sampling procedures. What is wavelength data for Granada? Do we include info for both illumination and reflectance, or remove info here?
Details of the statistical model for surface reflectance spectra are provided in the appendix. 
% JDB: make sure we have appendix.
...
% JDB: The next two sentences are confusing
...
% JDB: Here we get to contrast normalization. Should we call the first step luminance normalization?
