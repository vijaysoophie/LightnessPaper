...
Figure Blah [c] shows the first two AMA receptive fields.
...
%This is a more naturalistic condition compared to condition 1.
%Again, similar to condition 1, we used AMA to learn a set of linear receptive fields that are optimal for estimating target LRV for this condition.
%First we trained and evaluated AMA on the cone responses to the image patches.
Figure~\ref{fig:isomerizationFails} [a and b] show the results when we learned and evaluated the performance of AMA on the cone isomerizations.
Performance is poor.
...
value of the true LRV (asindicated by the width of the shaded red region).
...
Figure~\ref{fig:IsomerizationResults} [c and d] shows that performance improves greatly, and indeed is essentially perfect.
...
Figure~\ref{fig:IsomerizationResults} [a] shows the results.
...
Figure~\ref{fig:IsomerizationResults} [b] shows the responses of the first two AMA linear receptive fields for test [or training] stimuli.
...
%
%responses of the first two receptive fields to all the image patches in the training set.
...
%The decoding performance using these receptive fields for the test set is shown in .
%
...
%Thus, the AMA receptive fields learnt on the contrast normalized cone response (see Methods) can be used to predict the target LRV (. 
...
\centering
        \begin{subfigure}[b]{0.3 \textwidth}
        \caption{LRV estimates}
        \includegraphics[width=\textwidth]{../FiguresDraft4/Figure12/Figure12.pdf}
        \label{fig:case12Results}
    \end{subfigure}
    \caption{{\bf Condition 3:} Target LRV estimates v/s assigned luminance for images in test set of condition 3. The LRV estimates have been obtained using RFs learnt on contrast normalized images. The relative error of estimation is 22.9\%  for linear regression and 13.0\% for AMA.}
...
Fig.~\ref{fig:case12AllResults} shows the decoding performance for the test set in condition 3 obtained using the contrast normalized representation.
...
\begin{subfigure}{0.21 \textwidth}
	\caption{Condition 1}
	\includegraphics[width=\textwidth]{../FiguresDraft4/Figure13/Figure13_a.pdf}
	\label{fig:case1Filter}
    \end{subfigure}
    ~ ~ ~
    \begin{subfigure}{0.21 \textwidth}   
	\caption{Condition 2 isomerization}
	\includegraphics[width=\textwidth]{../FiguresDraft4/Figure13/Figure13_b.pdf}
	\label{fig:case2FilterI}
    \end{subfigure}
    ~ ~ ~
        \begin{subfigure}{0.21 \textwidth}
	\caption{Condition 2 contrast}
	\includegraphics[width=\textwidth]{../FiguresDraft4/Figure13/Figure13_c.pdf}
	\label{fig:case2FilterC}
    \end{subfigure}
    ~ ~ ~
        \begin{subfigure}{0.21 \textwidth}
	\caption{Condition 3}
	\includegraphics[width=\textwidth]{../FiguresDraft4/Figure13/Figure13_d.pdf}
	\label{fig:case3Filter}
    \end{subfigure}
\caption{{\bf AMA receptive fields:} The first two AMA receptive fields for the four conditions studied in this work. The receptive fields show a center surround structure. The L and M filters have larger weight as luminance is essentially a function of the L and M cone isomerizations. The stereotypical pattern in the receptive fields of condition 3 is due to the fixed geometry of the images.}
...
For condition 1, since the illuminant and the background are fixed, all the information is in the central target object reflectance. 
Thus, the receptive fields weights are mostly concentrated around the center (Figure~\ref{fig:case1Filter}). 
For condition 2, the illuminant spectra varies over several order of magnitude. 
As a result the photoreceptor isomerizations vary wildly. 
Thus, AMA receptive fields learnt on isomerizations do not show any clear pattern (Figure~\ref{fig:case2FilterI}). 
But, since the background object spectra is fixed, the contrast between the response 
of the cones corresponding to the target and the background carries the information about 
the target object LRV. 
The receptive fields learnt on the contrast signal shows this a trend. 
The receptive field weights are concentrated at the center similar to condition 1, there is large negative contribution from the surround (Figure~\ref{fig:case2FilterC}).
The spatial features of the RFs are due the stereotypical geometry of the images (see \nameref{Methods}). 
Condition 3 has a similar center surround structure, but with more prominent features of the background, since the spectra of the background objects are also varying (Figure~\ref{fig:case3Filter}). 
Again, the spatial features are a result of the stereotypical geometry of the images.
